...
class NFCUpdate extends StatefulWidget {
  const NFCUpdate({super.key});
  @override
  _NFCUpdateState createState() => _NFCUpdateState();
}
class _NFCUpdateState extends State<NFCUpdate> {
  ...
  void _writeData() async {
    startNfcSession(onTagDiscovered: (ndef) async {
      try {
        ...
        final birthDateRecord =
            NdefRecord.createText(_birthDateController.text);
        ...
        final message = NdefMessage([
          ...
          xRaysRecord, // 6 - X-rays and examinations (URL)
        ]);
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to write data')),
        );
      }
    });
  }
          ...
          child: Column(
            ...
            children: [
              ...
              CustomElevatedButton(
                text: getLang(context, "update_nfc_tagg"),
                onPressed: () {
